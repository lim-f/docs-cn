...
import {IS_DEV} from '../utils/compiler';
...
eveit.on('playground-code', ({name = 'Custom Code', code, isVue = false})=>{
    debugger;
    if(!code) {
        close();
...
let src = computed(() => {
    ...
    loading.value = true;
    const search = codeRef.value !== 'PLAYGROUND' ? 
        `?name=${nameRef.value}&code=${codeRef.value}&type=${isVueDemo ? 'vue': 'react'}`: '';
...
<template>
    <div v-show="codeRef !== ''" class="iframe-block">
        ...
        <div class="iframe-title">
            <span @click="close" style="color: #f66"><i class="ei-times"></i> Close</span>
